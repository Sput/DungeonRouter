...
The classifier has a 4,000-token output allowance and answer requests use an 8,000-token allowance. OpenAI counts reasoning and visible text within the same output limit; these ceilings provide room for reasoning without requiring the model to use every available token. The Nano classifier uses minimal reasoning effort, while the GPT-5 answer target uses low reasoning effort so each retains room for visible output. Empty model streams are treated as failures rather than successful blank answers. A zero-confidence Switchyard fall-through is labeled as a Mini safety fallback instead of classifier confidence.
